...
Those roots sit beside `~/.treehouse` rather than inside it, so a bare `treehouse status` or `treehouse prune` still reports exactly the primary's own pools: list `$HOME/.treehouse-homes/` to see which homes hold pools, and reclaim stale slots in one with `treehouse prune --all --yes --root $HOME/.treehouse-homes/<dir>`.
That prune is a dry run without `--yes`, so the flagless form lists what it would delete and reclaims nothing.
A private root is passed to Treehouse as `treehouse get --root`, which Treehouse grew in v2.2.0, so a home whose installed Treehouse is older cannot spawn a secondmate crewmate at all: the acquire is rejected outright and `fm-spawn.sh`'s worktree-isolation assertion refuses rather than launching a worker into the project itself.
...
A `https://github.com/<owner>/<repo>/pull/<n>` URL requires `gh` and `jq`, is merged only after one live read confirms the pull request is open, not a draft, mergeable, conflict-free, and every unwaived check it judges is green at the current head, then `gh pr merge` binds that verified head with `--match-head-commit`.
...
Which of those checks may refuse the merge is the base branch's own declaration, and `bin/fm-pr-merge.sh`'s `github_read_required_checks` owns that read: GitHub reports required status per check through the rollup's `isRequired`, which resolves classic branch protection and rulesets together and needs only ordinary repository read access, unlike the branch-protection and rules endpoints a repository's plan can refuse outright.
A declared required set narrows what is judged to exactly those checks, so a repository can run an advisory tier that fails on purpose without becoming unmergeable; a base branch that requires nothing and a read that failed both leave every check judged, because neither establishes what gates and a failed read must never shrink the judged set the way an empty one would.
Narrowing also cross-checks itself against GitHub's own `mergeStateStatus`, which the forge computes from the same requirements, so a pull request it still reports `BLOCKED` is refused even when every narrowed check was green, and a narrowed run reports which checks it judged and which it ignored while a run whose required read failed says so on its own `verified:` or refusal line.
That cross-check guards the narrowing, not an attended decision, so it stands aside — loudly, naming which one — when an `--allow-red` waiver covered a required check or `--attended-override` is in effect, because `BLOCKED` is the state those two exist to act on.
...
Allocation and return serialize on one project lock per pool per machine-local Firstmate tree: every home reachable through local parent links that shares a pool shares that lock, a home with its own private pool root takes its own lock because it reaches no other home's pool at all, and a home seeded from another machine anchors its own, because a lock taken on this filesystem is neither held nor observable across that boundary.
[`tests/fm-treehouse-home-pool-root.test.sh`](../tests/fm-treehouse-home-pool-root.test.sh) pins that the pool boundary and the lock boundary are derived from one fact, so homes with disjoint pools never refuse each other's allocations.
