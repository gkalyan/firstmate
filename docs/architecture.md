...
Those roots sit beside `~/.treehouse` rather than inside it, so a bare `treehouse status` or `treehouse prune` still reports exactly the primary's own pools: list `$HOME/.treehouse-homes/` to see which homes hold pools, and reclaim a stale slot inside a live home's root with `treehouse prune --all --root $HOME/.treehouse-homes/<dir>`.
Retiring a secondmate reclaims that home's own root as part of teardown, before the home, its lease, and its route are touched: it refuses and names the record if any task in any locally reachable home still records a worktree or home inside the root, so a retirement never removes a pool slot another task may own.
