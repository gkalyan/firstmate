...
Narrowing also cross-checks itself against GitHub's own `mergeStateStatus`, which the forge computes from the same requirements, so a pull request it still reports `BLOCKED` is refused even when every narrowed check was green, and every run reports which checks it judged and which it ignored.
